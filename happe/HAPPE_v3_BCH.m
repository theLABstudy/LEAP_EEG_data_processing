...
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
function HAPPE_v3_BCH( srcDir, reprocess, rerunExt, paramPath )

%% CLEAR THE WORKSPACE
clearvars -except srcDir reprocess rerunExt paramPath;
...
try
    ...
    fprintf(['Invalid input: please enter the complete path to the ' ...
            'folder containing the dataset(s).\n']) ;
end

%% DETERMINE IF USING PRESET PARAMETERS OR SET PARAMETERS
ver = '2_3_0' ;
% if input path to parameters, load; else, set from scratch
    ...
    try
        if ~reprocess
            ...
            try
                ...
                error('ERROR: Line noise reduction failed.',ME)
%                 fprintf(2, 'ERROR: Line noise reduction failed.\n') ;
            ...
            try
                ...
                % dataset.
                % GP ADDED 2023-03-07: address loading issue when using
                % different version of HAPPE
                try
                    ...
                    EEGloaded = pop_loadset('filename', ...
                        strrep(FileNames{currFile}, inputExt, ...
                        '_filtered.set'), 'filepath', [srcDir filesep ...
                        dirNames{contains(dirNames, 'intermediate_processing')}]) ;
                end
        ...
        if params.paradigm.ERP.on
            ...
            try
                ...
                if params.filt.butter
                    EEG.data = double(EEG.data); % LEAP: GP added 2022-10-04
        ...
        if params.segment.on
            try EEG = happe_segment_v4(EEG, params);
                if isempty(EEG), disp('No events found'), continue; end
